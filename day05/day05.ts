...
function part02() {
    ...
    function moveCratesMultiple(count: number, from: number, to: number, currStacks: string[][]): string[][] {
        let newStacks: string[][] = currStacks;
        let crate: string = '';
        ...
        for (let i=0; i<count; i++) {
            newStacks[to-1].push(cratesToMove[i]);
        }
    ...
    instructions.forEach( (instruction: number[]) => {
        finalStacks = moveCratesMultiple(instruction[0], instruction[1], instruction[2], finalStacks);
    } );
